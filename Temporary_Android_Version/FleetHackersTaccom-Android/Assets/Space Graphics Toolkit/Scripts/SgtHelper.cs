...
public static partial class SgtHelper
{
	...
	public static void WriteLights(List<Light> lights, int maxLights, Vector3 center, Transform directionTransform, Transform positionTransform)
	{
		var lightsCount = lights != null ? lights.Count : 0;
		var lightCount  = 0;
		
		for (var i = 0; i < lightsCount && lightCount < maxLights; i++)
		{
			var light = lights[i];
			...
			if (SgtHelper.Enabled(light) == true && light.intensity > 0.0f)
			{
				...
				for (var j = tempMaterials.Count - 1; j >= 0; j--)
				{
					...
					if (tempMaterial != null)
					{
						...
						tempMaterial.SetColor(prefix + "Color", color);
					}
				}
			}
		}
	...
	public static void WriteShadows(List<SgtShadow> shadows, int maxShadows)
	{
		...
		if (shadows != null)
		{
			...
			for (var i = 0; i < shadowsCount && shadowCount < maxShadows; i++)
			{
				...
				if (SgtHelper.Enabled(shadow) == true && shadow.CalculateShadow() == true)
				{
					...
					for (var j = tempMaterials.Count - 1; j >= 0; j--)
					{
						...
						if (tempMaterial != null)
						{
							...
							tempMaterial.SetMatrix(prefix + "Matrix", shadow.Matrix);
							tempMaterial.SetFloat(prefix + "Ratio", shadow.Ratio);
						}
					}
				}
			}
		...
		if (shadows != null)
		{
			...
			for (var i = 0; i < shadows.Count && shadowCount < maxShadows; i++)
			{
				...
				if (SgtHelper.Enabled(shadow) == true && shadow.CalculateShadow() == true)
				{
					...
					SetMatrix(prefix + "Matrix", shadow.Matrix);
				}
			}
		}
	}
